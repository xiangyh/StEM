...
inila<-mt[2]
  ...
  # using StEM as the initial value for EM algorithm
...
rm(theta)
  ...
  for(L in 1:100)
  {
    ...
    semem_weibull<-function(theta) {
      fn2<-function(y){
        ((y/theta[2])^theta[1])*dweibull(y,shape=mu1[L],scale=la[L])
      }
      en2 <- function(T1,T2){
        integrate(fn2,lower=T1,upper=T2)$value/(pweibull(T2,shape=mu1[L],scale=la[L])-pweibull(T1,shape=mu1[L],scale=la[L]))
      }
      lfem2<-0
      for(i in m){
        lf<-d[i]*en2(T[i],T[i+1])+r[i]*en2(T[i+1],Inf)
    ...
    if((abs((mu1[L+1]-mu1[L]))<10^(-5))&(abs((la[L+1]-la[L]))<10^(-5)))
      break
  }
  b<-1
},error=function(e){
